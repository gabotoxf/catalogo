...
<template>
  <div class="space-y-8 max-w-7xl mx-auto">
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <!-- Total Products -->
      <div
        class="bg-white p-5 rounded-2xl border border-neutral-200 shadow-sm flex flex-col justify-between hover:border-brand-500 transition-colors">
        <div class="flex justify-between items-center">
      ...
      <div
        ...
        <div class="mt-2 flex items-baseline gap-2">
          ...
          <span v-if="stats.stockBajo > 0"
            class="text-[10px] font-bold text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded">Revisar</span>
        </div>
      </div>
      ...
      <div
        ...
        <div class="mt-2">
          <h3 class="text-2xl font-bold text-neutral-900">${{ Math.floor(stats.valorInventario).toLocaleString('es-CO')
            }}</h3>
        </div>
      </div>
    </div>
    ...
    <div class="grid lg:grid-cols-12 gap-6">
      ...
      <div class="lg:col-span-8 space-y-4 min-w-0">
        <div class="flex justify-between items-center">
          ...
          <RouterLink to="/dashboard/productos"
            class="text-[11px] font-bold text-brand-600 hover:underline uppercase tracking-wider flex items-center gap-1">
            Ver catálogo
            <ArrowRight :size="12" />
          </RouterLink>
        </div>
        ...
        <div class="bg-white border border-neutral-200 rounded-2xl shadow-sm overflow-hidden">
          <div class="overflow-x-auto">
            <table class="w-full text-left text-xs min-w-[480px]">
              <thead
                class="bg-neutral-50 border-b border-neutral-100 text-neutral-400 uppercase text-[10px] font-bold tracking-widest">
                <tr>
                  <th class="px-4 py-4 sm:px-6">Producto</th>
                  <th class="px-4 py-4 sm:px-6 hidden sm:table-cell">Categoría</th>
                  <th class="px-4 py-4 sm:px-6 text-right">Precio</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-neutral-50">
                <tr v-for="prod in ultimosProductos" :key="prod.id_producto"
                    ...
                    <div class="flex items-center gap-3">
                      ...
                      <div class="min-w-0">
                        <span class="font-semibold text-neutral-900 truncate block max-w-[140px] sm:max-w-none">{{
                          prod.nombre_producto }}</span>
                        <!-- Categoría visible solo en mobile debajo del nombre -->
                        <span class="sm:hidden text-[10px] font-bold text-neutral-400 uppercase tracking-wider">
                          {{ prod.categoria?.nombre_categoria || 'S/C' }}
                        </span>
                      </div>
                    </div>
                  </td>
                  <td class="px-4 py-3 sm:px-6 hidden sm:table-cell">
                    <span
                      class="px-2 py-0.5 bg-neutral-100 text-neutral-500 rounded text-[10px] font-bold uppercase tracking-wider">
                      {{ prod.categoria?.nombre_categoria || 'S/C' }}
                    </span>
                  </td>
                  <td class="px-4 py-3 sm:px-6 font-bold text-neutral-900 text-right whitespace-nowrap">
                    ${{ Math.floor(prod.precio_producto).toLocaleString('es-CO') }}
                  </td>
                </tr>
      ...
      <div class="lg:col-span-4 space-y-4">
        ...
        <div class="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm space-y-5">
          <div v-for="cat in categoriasPopulares" :key="cat.id_categoria" class="space-y-1.5">
            ...
            <div class="h-1.5 bg-neutral-100 rounded-full overflow-hidden">
              <div class="h-full bg-brand-600 rounded-full"
                :style="{ width: `${(cat.productos_count / stats.totalProductos) * 100}%` }"></div>
            </div>
          </div>
        ...
        <div class="bg-brand-900 rounded-2xl p-6 text-white">
          ...
          <button
            class="w-full py-2 bg-white text-brand-900 rounded-lg font-bold text-xs hover:bg-neutral-100 transition-colors">Contactar</button>
        </div>
      </div>
    </div>
  </div>
